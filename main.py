...
def evaluate():
    ...
    try:
        ...
        try:
            ...
            for line in lines:
                # Normalize header line for comparison
                normalized_line = line.lower().replace("**", "").strip()

                # Start collecting when we hit the keywords section
                if "keywords for improvement" in normalized_line:
                ...
                if keyword_section_found:
                    ...
                    # Stop only if it's a new heading — not just a line with colon
                    # Added condition to stop if it's another section header like "Key Missing Areas"
                    if stripped == "" or normalized_line.startswith("key missing areas") or normalized_line.startswith("key areas"):
                         break # Stop if blank line or a new section heading

                    # Remove bullets/dashes if present
                    cleaned = stripped.lstrip("-*• ").strip()
                    # Only append if the cleaned line is not empty
                    if cleaned:
                         temp_keywords.append(cleaned)


            # Final clean-up: remove duplicates, filter short stuff
            # Further refinement to handle potential empty strings or very short words resulting from cleaning
            keywords_for_improvement = sorted(set([kw for kw in temp_keywords if len(kw) > 2]), key=len, reverse=True)[:5]


        except Exception as parse_err:
            print(f"Could not parse keywords for YouTube: {parse_err}") # You might keep this for debugging parsing issues

        # Fallback if needed
        if not keywords_for_improvement:
            # print("No specific keywords for improvement found/parsed in the analysis text.") # Comment out or remove this line
            keywords_for_improvement = ["resume writing tips", "job interview skills", "effective communication"] # Fallback

        # print(f"Keywords for YouTube: {keywords_for_improvement}") # Comment out or remove this line
        

        # --- Perform Youtube using courses.py ---
        # Only search if YOUTUBE_API_KEY is set
        if YOUTUBE_API_KEY:
